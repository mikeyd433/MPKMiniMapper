...
local function draw_device_config(x,y,w,h)
  ...
  local function rnf(cx,cy,lbl,val,mn,mx,step)
    rlbl(cx,cy,lbl); return num_field(cx+lw,cy,fw,16,val,mn,mx,step)
  end
  local function rcyc(cx,cy,lbl,idx,vals)
    rlbl(cx,cy,lbl); return cyc_field(cx+lw,cy,fw,16,idx,vals)
  end
  local function rtog(cx,cy,lbl,val)
    rlbl(cx,cy,lbl); return tog_field(cx+lw,cy,fw,16,val)
  end
  ...
  local new_oct = rnf(c1x,cy,"Octave",f.octave-4,-4,4);   cy=cy+RH
  f.octave = new_oct+4
  local new_tr = rnf(c1x,cy,"Transpose",f.transpose-12,-12,12); cy=cy+RH
  ...
  local new_ao = rnf(c2x,cy2,"Oct Range",f.arp_octave+1,1,4);             cy2=cy2+RH
