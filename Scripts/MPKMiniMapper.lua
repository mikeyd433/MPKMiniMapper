...
local S = {
  ...
  midi_device       = -1,
  midi_input        = nil,
  midi_read_fn      = nil,  -- cached API method name ("GetAllEvts" or "GetReadBuf")
...
local function midi_msg_size(sb)
  if not sb then return 1 end
  local hi = sb & 0xF0
  -- Program Change (0xC0) and Channel Pressure (0xD0): 2 bytes total
  if hi == 0xC0 or hi == 0xD0 then return 2 end
  -- Note Off/On, Aftertouch, CC, Pitch Bend: 3 bytes total
  if sb < 0xF0 then return 3 end
  -- System messages
  if sb == 0xF2 then return 3 end  -- Song Position
  if sb == 0xF3 then return 2 end  -- Song Select
  return 1  -- single-byte system realtime messages
end
...
local function open_midi_input()
  ...
  local mi=reaper.CreateMIDIInput(dev)
  if mi then S.midi_input=mi; status("Connected: "..name)
  else status("Could not open MIDI input for "..name) end
end

-- Read all pending events from the MIDI input object.
-- Tries GetAllEvts first (preferred), falls back to GetReadBuf.
-- Caches the working method name after the first successful call.
local function get_midi_buf(mi)
  if S.midi_read_fn=="GetAllEvts" then return mi:GetAllEvts()
  elseif S.midi_read_fn=="GetReadBuf" then return mi:GetReadBuf() end
  local ok,r=pcall(function() return mi:GetAllEvts() end)
  if ok and r~=nil then S.midi_read_fn="GetAllEvts"; return r end
  ok,r=pcall(function() return mi:GetReadBuf() end)
  if ok and r~=nil then S.midi_read_fn="GetReadBuf"; return r end
  return nil
end
...
-- Poll the hardware MIDI input for all pending events.
-- Buffer format: 4-byte timestamp | MIDI bytes (variable length per message type).
local function poll_midi(track)
  if not S.midi_input then return end
  local buf=get_midi_buf(S.midi_input)
  if not buf or buf=="" then return end

  local i=1
  while i<=#buf do
    if i+4>#buf then break end  -- need at least 4-byte timestamp + 1 status byte
    i=i+4  -- skip timestamp
    local sb=buf:byte(i)
    if not sb then break end
    -- Skip sysex (0xF0…0xF7) — variable length, not needed for this script
    if sb==0xF0 then
      while i<=#buf and buf:byte(i)~=0xF7 do i=i+1 end
      i=i+1
    else
      local mlen=midi_msg_size(sb)
      if i+mlen-1>#buf then break end
      local m1=sb
      local m2=(mlen>=2) and (buf:byte(i+1) or 0) or 0
      local m3=(mlen>=3) and (buf:byte(i+2) or 0) or 0
